...
const Subscriptions = () => {
    ...
    useEffect(() => {
        ...
        if(countStatus.current_month_visitors_count > planLimits[selected[0]]){
            setShowBanner(true)
        ...
        <Page title="Subscriptions">
            <Layout>
                <Layout.AnnotatedSection title="Billing" description="Select a different plan, click on Change Plan and approve the new subscription to change your plan.">
                    ...
                    {selectedState.billing_current_status === 'Expired' ? <Badge status="warning"><Heading element="h4">{selectedState.billing_plan_description}</Heading></Badge> : <Badge status="success"><Heading element="h4">{selectedState.billing_plan_description}</Heading></Badge>}<br/>
                    {selectedState.billing_current_status === 'Expired' ? <TextStyle variation="strong">Your current plan '{selectedState.billing_current_plan}' is Expired. Please choose a different plan:</TextStyle> : null}<br/><br/>
                    <ChoiceList
                    title="Select a plan"
                    choices={[
                        {...TrialPlan,disabled: selectedState.billing_current_status === 'Expired' || selectedState.billing_current_plan && (selectedState.billing_current_plan.includes('Basic') || selectedState.billing_current_plan.includes('Pro') || selectedState.billing_current_plan.includes('Premium')) },
                        {...BasicPlan, },
                        {...ProPlan},
                        {...PremiumPlan},
                    ]}
